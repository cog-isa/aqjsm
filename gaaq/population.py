...
class cpopulation:
    ...
    def fitness_function(self, bits):
        neg_cov, num_new_covered, num_all_not_miss, num_miss, coverage, exactcoverage, genes_has_ones = self.check_coverage(bits)
        if(neg_cov):
            return -np.inf
        num_ones = np.sum(bits)
        bigV = 1000.0
        if(num_new_covered!=0):
            fit = (num_all_not_miss * bigV) + (num_miss * 0.25 * bigV) + num_new_covered - (num_ones * 1.0 / bigV)#fit = num_uncov * 1000.0 + num0 - num_ones / 1000.0;
        else:
    ...
    def selection(self):
        def proportional(sorted_indexes):
            ...
            if(self.fits[sorted_indexes[0]] == self.fits[sorted_indexes[-1]]):
                ...
                self.fits[sorted_indexes[min_not_inf:]] = self.fits[sorted_indexes[min_not_inf:]] / self.fits[sorted_indexes[-1]]
                ...
                for i in range(min_not_inf+1,self.psize):
                    if(self.fits[sorted_indexes[i]]>0):
                        self.fits[sorted_indexes[min_not_inf:i]] = self.fits[sorted_indexes[i]]*1.0 / (i - min_not_inf)
                        if(min_not_inf!=0):
                            self.fits[sorted_indexes[0:min_not_inf]] = self.fits[sorted_indexes[min_not_inf]]*1.0 / min_not_inf
                        break
        ...
        def rang(sorted_indexes):
            ...
            self.parents = [sorted_indexes[int(np.ceil((-1 + np.sqrt(1.0 + 8.0 * (rand.randint(int(rangs_sum)) + 1))) / 2.0) - 1)] for j in range(2*self.psize)]
            return
        
        def tournament(sorted_indexes):
            self.parents = [sorted_indexes[max(rand.randint(self.psize) for i in range(self.tour_size))] for j in range(2*self.psize)]
            return
    ...
    def recombination(self):
        ...
        def twopointscut():
            ...
            for p_index in range(self.psize):
                ...
                if(p_index<self.psize*pop_part):
                    ...
                    self.children[p_index, cut[1]:self.numbit] = self.data[self.parents[2*p_index], cut[1]:self.numbit]
            del buff1
            del buff2
            return
        ...
        def uniform():
            ...
            for p_index in range(bound):
                buff1[0] = (self.data[self.parents[2*p_index]]&bits_to_get[p_index])+(self.data[self.parents[2*p_index+1]]&(np.invert(bits_to_get[p_index])+2))
                buff2[0] = (self.data[self.parents[2*p_index+1]]&bits_to_get[p_index])+(self.data[self.parents[2*p_index]]&(np.invert(bits_to_get[p_index])+2))
                if(self.fitness_function(buff1[0])>self.fitness_function(buff2[0])):
                    self.children[p_index] = buff1[0]
                else:
                    self.children[p_index] = buff2[0]
            if(bound!=self.psize):
                #choose just first child, not necessarily the best
                self.children[bound:self.psize] = (self.data[self.parents[2*bound::2]]&bits_to_get[bound:self.psize])+(self.data[self.parents[2*bound+1::2]]&(np.invert(bits_to_get[bound:self.psize])+2))
            del buff1
            del buff2
            return
        ...
        if (self.sel_type == 0):
            ...
            for i in range(gsize):
                if (self.fits[sorted_indexes[i]] != -np.inf):
                    ...
                    sum1 += 1
                    break
            probsel = np.array([100.0 * (self.fits[sorted_indexes[i]] + dbuff) / sum1 if self.fits[sorted_indexes[i]] != -np.inf  else 1.0 / sum1 for i in range(gsize)])
        if (self.sel_type == 1):
            ...
            probsel = np.array([(((gsize - i) ** self.tour_size) - ((gsize - i - 1) ** self.tour_size)) * 1.0 / denom for i in range(gsize)])
...
def run_gaaq(processed_data, numval_per_gen, pos_index, neg_index, uncov_pos):
    ...
    pop.init(gap.pops_size, processed_data, numval_per_gen, pos_index, neg_index, uncov_pos,
             gap.ga_type[2], gap.sel_type[2], gap.tour_size[2], gap.elitism[2], gap.rec_type[2], gap.pop_part_rec[2], gap.nmut_bits[2], gap.pop_part_distr[2])
    ...
    if(len(pop.bestgenotype != 0)):
        neg_cov, num_new_covered, num_all_not_miss, num_miss, rule_coverage, rule_exactcoverage, genes_has_ones = pop.check_coverage(pop.bestgenotype)
    else:
        neg_cov = num_new_covered = num_all_not_miss = num_miss = 0
        rule_coverage = []
        rule_exactcoverage = []
