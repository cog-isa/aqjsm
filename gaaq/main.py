...
from gaaq.coevolution import run_coev_gaaq
...
def preprocess_data(dataset, NumRealIntervals):
    ...
    numval_per_gen = np.array([len(attr[1]) if attr[1] != 'REAL' else NumRealIntervals for attr in
    ...
    for i, aNeg in enumerate(processed_data[neg_index]):
        ...
        for aPos in processed_data[pos_index]:
            ...
            if (found):
                to_del.append(i)
                break
    print(("Found and deleted %d conflict negative objects (" % len(to_del)) + (
        " ".join(str(k) for k in neg_index[to_del])) + ")")
...
def evaluate_AQ_rules(processed_data, class_labels, numval_per_gen):
    ...
    for classLabel in labels_set:
        ...
        while (len(uncov_pos) != 0):
            ...
            # Coevolutionary GA
            neg_cov, num_new_covered, num_all_not_miss, num_miss, rule_coverage, rule_exactcoverage, genes_has_ones, best_bits, best_fit = run_coev_gaaq(
                processed_data, numval_per_gen, pos_index, neg_index, uncov_pos)
        ...
        for i in range(len(rule_genotypes)):
            result += "RULE_%d:\n" % (i + 1)
            result += ("\tNUM_NEW_OBJECTS: %d (" % (num_new_objects[i])) + " ".join(
                str(k) for k in newobj_cov_by_rule[i]) + ")\n"
            result += ("\tNUM_OBJECTS: %d (" % (num_objects[i])) + " ".join(str(k) for k in obj_cov_by_rule[i]) + ")\n"
            result += ("\tNUM_EXACT_OBJECTS: %d (" % (num_objects[i] - num_miss_objects[i])) + " ".join(
                str(k) for k in exactobj_cov_by_rule[i]) + ")\n"
            ...
            for j in essential[i]:
                ...
                ones_pos = [k + 1 for k in range(len(genbits)) if
                            (genbits[k] == 1)]  # k+1 because positions in AQRules are numerated from 1
                rule.features.append((j,
                                      ones_pos))  # rule.getTokens().put(featureMap.get(i), ones_pos_limited); # no need to evaluate ones_pos_limited now
                result += ("\tattr_%d = " % (j)) + " ".join(
                    str(k) for k in ones_pos) + "\n"  # attributes for some reason are numerated from 0
