...
class ccoevolution:
    def __init__(self, processed_data, numval_per_gen, pos_index, neg_index, uncov_pos):
        ...
        for i in range(self.cn-1):
            p = population.cpopulation()
            p.init(int(self.n / self.cn), processed_data, numval_per_gen, pos_index, neg_index, uncov_pos, 
                   gap.ga_type[i], gap.sel_type[i], gap.tour_size[i], gap.elitism[i], gap.rec_type[i], gap.pop_part_rec[i], gap.nmut_bits[i], gap.pop_part_distr[i])
            self.pop.append(p)
        p = population.cpopulation()
        p.init(int(self.n / self.cn)+int(self.n % self.cn), processed_data, numval_per_gen, pos_index, neg_index, uncov_pos, 
                   gap.ga_type[-1], gap.sel_type[-1], gap.tour_size[-1], gap.elitism[-1], gap.rec_type[-1], gap.pop_part_rec[-1], gap.nmut_bits[-1], gap.pop_part_distr[-1])
        self.pop.append(p)
        ...
        self.generalgroup = population.cpopulation()
    ...
    def moveToGeneralGroup(self):
        ...
        sorted_indexes = sorted(sorted_indexes, key = lambda i: -self.pop[data_indexes[i][0]].fits[data_indexes[i][1]]) # descending order     
    ...
    def moveOutOfGeneralGroup2(self, generalgroup, generalfits):
        for p in self.pop:
            ...
            if (p.psize < p.prevpsize):
                p.data[sorted_indexes[(p.psize-int(self.socialcard/2.0)):p.psize]] = np.copy(generalgroup[:int(self.socialcard/2.0)])
                p.fits[sorted_indexes[(p.psize-int(self.socialcard/2.0)):p.psize]] = np.copy(generalfits[:int(self.socialcard/2.0)])
                ...
                p.data[sorted_indexes[(p.prevpsize-int(self.socialcard/2.0)):p.prevpsize]] = np.copy(generalgroup[:int(self.socialcard/2.0)])
                p.fits[sorted_indexes[(p.prevpsize-int(self.socialcard/2.0)):p.prevpsize]] = np.copy(generalfits[:int(self.socialcard/2.0)])
            if p.bestgenotypefit < generalfits[0]:
                p.bestgenotype = np.copy(generalgroup[0])
                p.bestgenotypefit = generalfits[0]
            if self.coevbestgenotypefit < p.bestgenotypefit:
...
def run_coev_gaaq(processed_data, numval_per_gen, pos_index, neg_index, uncov_pos):
    ...
    if(len(coev_pop.coevbestgenotype != 0)):
        neg_cov, num_new_covered, num_all_not_miss, num_miss, rule_coverage, rule_exactcoverage, genes_has_ones = coev_pop.pop[0].check_coverage(coev_pop.coevbestgenotype)
    else:
